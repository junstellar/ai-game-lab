...
SH.Game = (function () {
  ...
  function pickStartLevel() {
    var pack = levelPack();
    var saved = shotMode ? null : U.store(SAVE_KEY);
  ...
  function enterPlay() {
    ...
    setPhase('PLAY');
    if (curIndex === 0 && def && !progGet(def.id)) M('UI', 'tutorial', 0);
  }
  ...
  function doGo(id) {
    enterPlay();
    if (phase !== 'PLAY' || busy || !st || !id) return false;
  ...
  function doCut(k) {
    enterPlay();
    if (phase !== 'PLAY' || busy || !st) return false;
    k = k | 0;
